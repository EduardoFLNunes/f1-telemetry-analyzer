...
class AssettoAdapter:
    ...
    def _normalize(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vertical_g": self.physics.accG[1],
            "velocity": {
                "x": self.physics.velocity[0],
                "y": self.physics.velocity[1],
                "z": self.physics.velocity[2],
            },
            "wheel_slip": list(self.physics.wheelSlip),
            "wheel_load": list(self.physics.wheelLoad),
            "wheels_pressure": list(self.physics.wheelsPressure),
            "tyre_wear": list(self.physics.tyreWear),
            "tyre_dirty_level": list(self.physics.tyreDirtyLevel),
            "tyre_core_temperature": list(self.physics.tyreCoreTemp),
            "suspension_travel": list(self.physics.suspensionTravel),
            "ride_height": list(self.physics.rideHeight),
            "camber_rad": list(self.physics.camberRAD),
            "fuel": self.physics.fuel,
            "max_fuel": self.static.maxFuel,
            "ballast": self.physics.ballast,
            "car_damage": list(self.physics.carDamage),
            "abs": self.physics.abs,
            "tc": self.physics.tc,
            "drs": bool(self.physics.drs),
            "turbo_boost": self.physics.turboBoost,
            "air_density": self.physics.airDensity,
            "air_temp": self.physics.airTemp,
            "road_temp": self.physics.roadTemp,
            "surface_grip": self.graphics.surfaceGrip,
            "timestamp": time.time(),
